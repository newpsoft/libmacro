...
{
	return mcr_Instance_deinit(signalPt);
}
...
int mcr_Signalref_compare(const void *lhsPtPt, const void *rhsPtPt)
{
	if (rhsPtPt) {
		if (lhsPtPt) {
			if (lhsPtPt == rhsPtPt)
				return 0;
			return mcr_Signal_compare(*(const void * const*)lhsPtPt,
...
int mcr_signal_deinitialize(struct mcr_context *ctx)
{
	...
	mcr_Map_deinit(&modSignal->map_modifier_name);
	mcr_Map_deinit(&modSignal->map_name_modifier);
